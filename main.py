...
from PyQt6.QtWidgets import (
    ...
    QLabel, QMessageBox, QInputDialog, QSplitter, QStackedWidget,
    QListWidgetItem, QFrame, QSizePolicy, QDialog, QScrollArea
)
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, QObject, QEvent
from PyQt6.QtGui import QWindow, QTextCursor, QPalette, QColor, QIcon
...
def find_scrcpy_hwnd(pid, exclude=()):
    if not IS_WIN:
        return None
...
class MirrorWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        side_panel.setFixedWidth(MIRROR_PAGE_BASE_W + 100)
        ...
        self.nav_list.setFixedWidth(88)
        ...
        self.mirror_layout.setContentsMargins(10, 10, 10, 10)

        self.mirror_embed_holder = QWidget()
        self.mirror_embed_holder.setStyleSheet("background: #000000; border-radius: 8px;")
        ...
        self.mirror_layout.addWidget(self.mirror_stack, 1)

        splitter.addWidget(self.mirror_container)
        splitter.setSizes([300, 620])
        splitter.setStretchFactor(0, 0)
        splitter.setStretchFactor(1, 1)
        splitter.splitterMoved.connect(self._update_mirror_size)
        self.splitter = splitter
        layout.addWidget(splitter)
        self.statusBar().showMessage("谷歌框架远程操作 · 精简投屏")

        self.mirror_container.installEventFilter(self)
        self.mirror_embed_holder.installEventFilter(self)
        self.mirror_stack.installEventFilter(self)

    def eventFilter(self, obj, event):
        if event.type() == QEvent.Type.Resize and obj in (
            self.mirror_container, self.mirror_embed_holder, self.mirror_stack
        ):
    ...
    def _container_size(self):
        cw = self.mirror_stack.width()
        ch = self.mirror_stack.height()
        if cw < 50 or ch < 50:
            cw = self.mirror_container.width()
            ch = self.mirror_container.height()
        return cw, ch

    def _mirror_display_size(self):
        cw, ch = self._container_size()
        return calc_fit_rect(cw, ch, self.device_w, self.device_h)

    def _update_mirror_size(self):
        if not self.scrcpy_embed_widget or not self.mirroring:
            return
        _, _, w, h = self._mirror_display_size()
        if w < 50 or h < 50:
            return
        self.scrcpy_embed_widget.setFixedSize(w, h)
        if self.scrcpy_qwindow:
            self.scrcpy_qwindow.resize(w, h)
    ...
    def _try_embed_scrcpy(self):
        ...
        hwnd = find_scrcpy_hwnd(self.scrcpy_proc.pid, exclude=(int(self.winId()),))
        if not hwnd:
        ...
        try:
            ...
            self.scrcpy_qwindow = qwin
            self.scrcpy_hwnd = hwnd
            embed = QWidget.createWindowContainer(qwin, self.mirror_embed_holder)
            embed.setFocusPolicy(Qt.FocusPolicy.StrongFocus)
            embed.setSizePolicy(
                QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed
            )
            self.scrcpy_embed_widget = embed
            self.mirror_holder_layout.addWidget(
                embed, 0, Qt.AlignmentFlag.AlignCenter
            )
            self.mirroring = True
            self._update_mirror_size()
            QTimer.singleShot(80, self._update_mirror_size)
            QTimer.singleShot(300, self._update_mirror_size)
            self.embed_timer.stop()
            self.proc_timer.start(500)
            self.log("投屏画面已嵌入右侧区域（比例自适应）")
    ...
    def start_mirror(self):
        ...
        if IS_MAC:
            ...
            cw = max(self.mirror_container.width(), 300)
            ch = max(self.mirror_container.height(), 400)
            _, _, ww, wh = calc_fit_rect(cw, ch, self.device_w, self.device_h)
            args = [
                str(SCRCPY), "-s", self.current_serial,
                "--max-fps", "60", "--video-bit-rate", "8M",
                "--window-title", f"scrcpy_embed_{self.current_serial}",
                "--window-borderless",
                "--max-size", str(max(ww, wh, 1920)),
                "--window-width", str(ww),
    ...
    def showEvent(self, event):
        ...
        QTimer.singleShot(120, self._update_mirror_size)
